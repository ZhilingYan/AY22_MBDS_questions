...
parser.add_argument('--image_dir', default='../dataset', help='Image directory for tumor patches', dest='image_dir')
...
parser.add_argument('--num_instances', default='10', type=int, help='number of instances (patches) in a bag', dest='num_instances')
parser.add_argument('--num_features', default='100', type=int, help='number of features', dest='num_features')
...
parser.add_argument('--num_epochs', default=20, type=int, help='number of steps of execution (default: 1000000)', dest='num_epochs')
parser.add_argument('--save_interval', default=1, type=int, help='model save interval (default: 1000)', dest='save_interval')
...
val_dataset = Dataset(image_dir=FLAGS.image_dir, normal_image_dir=FLAGS.normal_image_dir, dataset_dir=FLAGS.dataset_dir, dataset_type='test', patch_size=FLAGS.patch_size, fold_list=[FLAGS.valid_fold], num_instances=FLAGS.num_instances)
# num_patients_val = val_dataset.num_patients
# print("Validation Data - num_patients: {}".format(num_patients_val))

train_data_loader = torch.utils.data.DataLoader(train_dataset, batch_size=FLAGS.batch_size, shuffle=True, num_workers=4, collate_fn=custom_collate_fn, worker_init_fn=worker_init_fn, drop_last = True)
val_data_loader = torch.utils.data.DataLoader(val_dataset, batch_size=FLAGS.batch_size, shuffle=False, num_workers=4, collate_fn=custom_collate_fn, worker_init_fn=worker_init_fn, drop_last = True)
...
for epoch in range(FLAGS.num_epochs):
	...
	for images, targets in train_data_loader:
		...
		print(y_logits[:10], targets[:10])

		loss = criterion(y_logits, targets.long())
		# loss = F.nll_loss(y_logits, targets)
